...
export const GET: APIRoute = async (context) => {
  ...
  if (!apiKey || !username) {
    return new Response(
      JSON.stringify({ track: null, statusText: "Server Misconfiguration" }),
      {
        status: 500,
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return new Response(
        JSON.stringify({ track: null, statusText: "API Error" }),
        {
          status: res.status,
          headers: cacheHeaders,
        },
    ...
    if (
      ...
      const albumArt =
        trackData.image.find((img) => img.size === "extralarge") ||
      ...
      return new Response(
        JSON.stringify({ track: trackResult, statusText: statusText }),
        {
          status: 200,
          headers: cacheHeaders,
        },
      ...
      return new Response(
        JSON.stringify({ track: null, statusText: statusMessage }),
        {
          status: 200,
          headers: cacheHeaders,
        },
    ...
    return new Response(
      JSON.stringify({ track: null, statusText: "API Fetch Error" }),
      {
        status: 503,
        headers: cacheHeaders,
      },
